...
public class AccountController {
    ...
    @Operation(summary = "가계부 메모 수정", description = "특정 가계부의 메모를 수정합니다.")
    public Messenger updateMemo(
        ...
        try {
            ...
            return accountService.updateMemo(id, userId, memo);
        } catch (Exception e) {
            return Messenger.builder()
                    .code(500)
                    .message("메모 수정 실패: " + e.getMessage())
                    .build();
        }
    }

    @PatchMapping("/{id}/alarm")
    @Operation(summary = "가계부 알람 설정", description = "특정 가계부의 알람을 설정합니다.")
    public Messenger updateAlarm(
            @PathVariable Long id,
            @RequestBody Map<String, Object> request,
            @RequestHeader(value = "Authorization", required = false) String authHeader) {
        try {
            Boolean alarmEnabled = request.get("alarmEnabled") != null ? 
                Boolean.parseBoolean(request.get("alarmEnabled").toString()) : null;
            String alarmDateStr = (String) request.get("alarmDate");
            String alarmTimeStr = (String) request.get("alarmTime");
            ...
            java.time.LocalDate alarmDate = alarmDateStr != null ? 
                java.time.LocalDate.parse(alarmDateStr) : null;
            java.time.LocalTime alarmTime = alarmTimeStr != null ? 
                java.time.LocalTime.parse(alarmTimeStr) : null;
            
            return accountService.updateAlarm(id, userId, alarmEnabled, alarmDate, alarmTime);
        } catch (Exception e) {
            return Messenger.builder()
                    .code(500)
                    .message("알람 설정 실패: " + e.getMessage())
                    .build();
        }
    }

    @GetMapping("/alarms")
    @Operation(summary = "활성 알람 조회", description = "JWT 토큰에서 사용자 ID를 추출하여 활성화된 알람 목록을 조회합니다.")
    public Messenger findActiveAlarms(
        ...
        return accountService.findActiveAlarms(userId);
    }
