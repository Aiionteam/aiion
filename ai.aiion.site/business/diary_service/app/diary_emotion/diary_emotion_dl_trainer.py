...
class DiaryEmotionDLTrainer:
    ...
    def create_dataloader(
        ...
        # Docker 컨테이너에서는 멀티프로세싱 문제가 발생할 수 있으므로 환경 변수로 제어
        import platform
        import os
        
        # 환경 변수로 num_workers 제어 (기본값: 자동 감지)
        env_num_workers = os.getenv('DATALOADER_NUM_WORKERS')
        if env_num_workers is not None:
            num_workers = int(env_num_workers)
            ic(f"DataLoader num_workers 설정: {num_workers} (환경 변수에서 설정)")
        else:
            # Docker 컨테이너 내부인지 확인
            is_docker = os.path.exists('/.dockerenv')
            is_linux = platform.system() == "Linux"
            
            # Docker 컨테이너에서는 멀티프로세싱 문제가 발생할 수 있으므로 0으로 설정
            if is_docker:
                num_workers = 0  # Docker 컨테이너에서는 멀티프로세싱 비활성화
                ic(f"DataLoader num_workers 설정: {num_workers} (Docker 컨테이너 환경)")
            elif is_linux and platform.system() != "Windows":
                # 로컬 Linux 환경에서는 멀티프로세싱 사용
                try:
                    import multiprocessing
                    num_workers = min(multiprocessing.cpu_count(), 4)  # 최대 4개로 제한
                except:
                    num_workers = 2
                ic(f"DataLoader num_workers 설정: {num_workers} (Linux 환경, 멀티프로세싱 활성화)")
            else:
    ...
    def train(
        ...
        Args:
            ...
            label_smoothing: Label smoothing 값 (0.0 = 비활성화, 0.1 = 권장값, 과적합 방지)
        ...
        # 손실 함수 (클래스 불균형 처리 + Label Smoothing)
        # 라벨 분포 계산
        ...
        if label_smoothing > 0.0:
            ic(f"✅ Label Smoothing 활성화: {label_smoothing} (과적합 방지)")
