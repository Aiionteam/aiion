...
// AI 서버 게이트웨이 설정
// 통합 게이트웨이 사용: 8080 포트 (api.aiion.site의 Spring Cloud Gateway)
export const AI_GATEWAY_CONFIG = {
  HOST: process.env.NEXT_PUBLIC_AI_GATEWAY_HOST || process.env.AI_GATEWAY_HOST || 'localhost',
  PORT: process.env.NEXT_PUBLIC_AI_GATEWAY_PORT || process.env.AI_GATEWAY_PORT || '8080',
  BASE_URL: `http://${process.env.NEXT_PUBLIC_AI_GATEWAY_HOST || process.env.AI_GATEWAY_HOST || 'localhost'}:${process.env.NEXT_PUBLIC_AI_GATEWAY_PORT || process.env.AI_GATEWAY_PORT || '8080'}`,
} as const;
